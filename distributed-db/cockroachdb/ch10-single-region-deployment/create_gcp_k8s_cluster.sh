gcloud container clusters create crdb --zone=us-central1-c \ 
    --machine-type n2-standard-4 --disk-type=pd-ssd \ 
    --disk-size=200GB --project=just-rhythm-211204;
...
# It’s handy to have a “client” pod in the cluster that is able to issue cockroach sql
# commands. The following command creates such a pod:
...
# For a database connection, we are going to need at least the CA certificate. We can
# copy that from one of the CockroachDB pods:
rm -rf certs;
mkdir certs;
