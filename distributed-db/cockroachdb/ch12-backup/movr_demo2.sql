SELECT raw_config_sql FROM crdb_internal.zones WHERE target='RANGE default';
...
SELECT raw_config_sql FROM crdb_internal.zones WHERE target='DATABASE movr';

SELECT name,locality FROM crdb_internal."tables" 
...
SELECT  job_type,description,status FROM [show jobs] WHERE description LIKE '%REGIONAL%';
...
SELECT DISTINCT index_name,column_Name FROM crdb_internal.index_columns  
...
SHOW RANGE FROM INDEX users@primary FOR ROW (
   'gcp-us-east1','new york','00000000-0000-4000-8000-000000000000'
);

SHOW RANGE FROM INDEX users@primary FOR ROW (
  'gcp-europe-west1','amsterdam','ae147ae1-47ae-4800-8000-000000000022'
);
